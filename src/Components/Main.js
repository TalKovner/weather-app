...
class Main extends Component {
  ...
  removeCity(cityToRemove) {
    this.setState((prevState) => {
      let newCities = prevState.citiesName.filter(
  ...
  render() {
    let cardsComponents = this.state.citiesName.map((currentCity) => (
      <Card
        key={currentCity}
        city={currentCity}
    ...
    return (
      <div className="main">
        ...
        {!this.state.showCards ? (
          <BackButton back={this.toggleCards.bind(this)} />
        ) : null}
        {!this.state.showCards ? (
          <CityDetails city={this.state.cityOnFocus} />
        ) : null}
      </div>
    );
  }
