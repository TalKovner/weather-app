...
class WeeklyWeather extends Component {
    ...
    componentDidMount() {
        ...
        axios.get(apiUrl).then(res => {
            ...
            averageData = dataPerDay.map((dayDataArray, index) => {
                ...
                dayAverage.temp = ((avarageTemp / dayDataArray.length) - 272.15).toFixed(0);
                dayAverage.condition = handleCondition(avarageContitionID / dayDataArray.length);
                dayAverage.day = new Date(new Date().getTime() + (millSecInDay * (index + 1))).toDateString().split(' ')[0];
    ...
    render() {
        const daysComponents = this.state.daysData.map(dayData => <DailyWeather data={dayData}/>);
        return (
